...
import deu.rest.api.Service.CampusMapService;
...
public class CampusMapController {
    ...
    @GetMapping("/campusMap/fetch")  // HTTP POST 요청을 "/fetch" 경로로 받을 때 실행됨
    public ResponseEntity<String> fetchColleges() {
        try {
            campusMapService.fetchAndSaveCampusMap();  // 크롤링 + DB 저장 수행
            return ResponseEntity.ok("캠퍼스맵 데이터를 성공적으로 저장했습니다.");  // 200 OK 응답
        } catch (IOException e) {
